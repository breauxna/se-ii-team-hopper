...
(module MSplitcolor
  ...
  (defun splitcolor (img)
    (if (and (not (is-image-empty? img))
             (< 0 (img-width img)) 
             (< 0 (img-height img)))
        (list (get-red-tree img (empty-image (img-width img) (img-height img)) 0 0) 
              (get-green-tree img (empty-image (img-width img) (img-height img)) 0 0) 
              (get-blue-tree img (empty-image (img-width img) (img-height img)) 0 0))
        img))
