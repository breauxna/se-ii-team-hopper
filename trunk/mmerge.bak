...
(module MMerge
  ...
  ;merge-up merges img2 to the top of img1
  ;@param img1 - image at the bottom
  ;@param img2 - image on top
  ;@param x - current x location in img2
  ;@param y - current y location in img2
  ;@return img - newly merged image
  (defun merge-up (img1 img2 x y)
    (if (is-image-empty? img1)
        img2
        (if (is-image-empty? img2)
            img1
        ...
        (if (is-image-empty? img2)
            ...
            (let* ((w1 (img-width img1))
              ...
              (cond
                ((equal dir 'up)
                 (if (equal w1 w2)
                     (change-size w1 (+ h1 h2) (merge-up img1 img2 0 0))
                     img1))
                ((equal dir 'down)
                 (if (equal w1 w2)
                     (change-size w1 (+ h1 h2) (merge-up img2 img1 0 0))
                     img1))
                ((equal dir 'left)
                 (if (equal h1 h2)
