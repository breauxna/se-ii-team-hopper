...
(module THue
  ...
  ;Checks if increasing hue by 360 degrees
  ;returns the original image
  (defproperty hue-round-trip :repeat 100
    (width  :value (random-between 1 10)
     height :value (random-between 1 10)
     img    :value (random-image width height))
    (image-equal? img (hue img 360)))
  
  ;Checks if after doing hue that the
  ;size of image is the same
  (defproperty hue-same-size :repeat 100
    (width  :value (random-between 1 10)
     height :value (random-between 1 10)
     b      :value (random-natural)
     img    :value (random-image width height))
    (let ((new-img (hue img b)))
