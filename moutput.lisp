...
(module MOutput
  ...
  ;The size of the pixel array plus the header. The header is always 54 
  ;bytes. The pixel array is made up of each pixel with 3 bytes and junk 
  ;zeroes between each column in compliance with the standard. There are 
  ;also 2 zeroes appended to the end
  ...
  (defun get-size-image-word (width height)
    (pad-right (num->word (+ (* 3 width height) 
                             (+ (* (junk-width width) height) 2)) nil) 4))
  ...
  ;Formats the pixel array. You feed to it the image, the header, 
  ;and two zeroes
    ...
    (if (not (is-image-empty? img))
        (if (< y (img-height img))
            (if (< x (img-width img))
                ...
                (get-pixel-array img 
                                 (pad-right bytes 
                                            (+ (junk-width (img-width img))
                                               (len bytes))) 0 (1+ y)))
            (append bytes (list 0 0)))
        (append bytes (list 0 0))))
  
  ;Writes a bmp to a path location
  ...
  ;Wrapper function for writing an image to a file location
  (defun write-bmp-file (img path)
    (w-bmp (get-pixel-array img 
                            (get-header (img-width img) 
                                        (img-height img)) 
                            0 
                            0) path state))
